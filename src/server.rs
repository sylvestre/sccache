    ...
    C: CommandCreatorSync + Clone + Send + Sync + 'static,
{
    ...
    async fn handle_cache_get(
        ...
        match self.storage.get(&req.key).await {
            Ok(Cache::Hit(mut cache_read)) => {
                ...
                match cache_read
                    .extract_objects(req.output_paths, &self.rt)
                    .await
                {
                    ...
                    Err(e) => {
                        warn!("CacheGet({}): failed to extract cache entry: {e:#}", req.key);
            ...
            Ok(Cache::Miss) | Ok(Cache::None) | Ok(Cache::Recache) => {
                Ok(Message::WithoutBody(Response::CacheGet(CacheGetResponse::Miss)))
            }
            Err(e) => {
    ...
    async fn handle_cache_put(
        ...
        let mut entry = match CacheWrite::from_objects(req.output_paths, &pool).await {
            Ok(e) => e,
            Err(e) => {
                warn!("CachePut({}): failed to read output files from disk: {e:#}", req.key);
                // Best-effort: don't crash the connection on a put failure.
        ...
        // Ignore storage errors – this is best-effort caching.
        let duration = match self.storage.put(&req.key, entry).await {
            Ok(d) => d,
            Err(e) => {
                warn!("CachePut({}): failed to store to backend: {e:#}", req.key);
                std::time::Duration::default()
            }
        };
